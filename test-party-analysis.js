...
function displayResults(role, analysis) {
  ...
  console.log(
    `Total: ${analysis.summary.totalRisks}, High: ${analysis.summary.highRisks}, Medium: ${analysis.summary.mediumRisks}`,
  );
...
function compareAnalyses(employeeAnalysis, employerAnalysis) {
  ...
  const employeeCategories = new Set(
    employeeAnalysis.risks.map((r) => r.category),
  );
  const employerCategories = new Set(
    employerAnalysis.risks.map((r) => r.category),
  );

  console.log(
    "Employee risk categories:",
    Array.from(employeeCategories).join(", "),
  );
  console.log(
    "Employer risk categories:",
    Array.from(employerCategories).join(", "),
  );
  ...
  const employeeNonCompete = employeeAnalysis.risks.find(
    (r) =>
      r.title.toLowerCase().includes("non-compete") ||
      r.description.toLowerCase().includes("non-compete"),
  );
  const employerNonCompete = employerAnalysis.risks.find(
    (r) =>
      r.title.toLowerCase().includes("non-compete") ||
      r.description.toLowerCase().includes("non-compete"),
  );
  ...
  if (employeeNonCompete && !employerNonCompete) {
    console.log(
      "- Non-compete is a risk for Employee but not flagged for Employer",
    );
  } else if (!employeeNonCompete && employerNonCompete) {
    console.log(
      "- Non-compete is a risk for Employer but not flagged for Employee",
    );
  }

  // Check termination clause perspectives
  const employeeTermination = employeeAnalysis.risks.find(
    (r) =>
      r.category === "Termination" ||
      r.title.toLowerCase().includes("terminat"),
  );
  const employerTermination = employerAnalysis.risks.find(
    (r) =>
      r.category === "Termination" ||
      r.title.toLowerCase().includes("terminat"),
  );
...
console.log(
  "2. Open this file in a browser console or use a tool like Postman",
);
