...
import { getTemplatePromptAdditions, getContractTemplate, getPartySpecificRedFlags } from './contract-templates';
...
export async function analyzeContractWithAI(
  ...
  try {
    // Get contract-specific template additions
    const templateAdditions = getTemplatePromptAdditions(contractType, partyRole);
    
    // Create a focused prompt for Claude with template guidance
...
export async function analyzeContractClauses(
  ...
  try {
    // Get contract-specific template guidance
    const template = getContractTemplate(contractType);
    const templateAdditions = getTemplatePromptAdditions(contractType, partyRole);
    
    // Add critical clauses guidance if template exists
    const criticalClausesGuidance = template ? 
