...
// Dynamic import for pdf-parse-new to avoid build issues
async function parsePDF(buffer: Buffer): Promise<string> {
  try {
    ...
    if (error && typeof error === "object" && "message" in error) {
      ...
      if (
        errorMessage.includes("test/data") ||
        errorMessage.includes("versions-space.pdf")
      ) {
        throw new Error(
          "PDF parsing is currently unavailable. Please convert your PDF to .txt or .docx format for analysis.",
        );
      }
    }

    throw new Error(
      "Failed to parse PDF file. Please try converting to .txt or .docx format.",
    );
  }
}
...
export async function extractTextFromFile(file: File): Promise<string> {
  ...
  try {
      ...
      try {
        ...
        if (result.value && result.value.trim()) {
          ...
          if (result.value.length < 50) {
            console.warn(
              "Extracted text seems too short, document might be empty",
            );
          }
    ...
    if (fileType === "application/msword" || fileName.endsWith(".doc")) {
      throw new Error(
        "Old .doc format is not fully supported. Please save your document as .docx or .txt format",
      );
    }
    ...
    if (fileType === "application/pdf" || fileName.endsWith(".pdf")) {
      try {
        ...
        if (text && text.trim()) {
          console.log(
            `Successfully extracted ${text.length} characters from PDF`,
          );
          return text;
        } else {
        ...
        throw new Error(
          "PDF parsing is not supported at this time. Please convert your PDF to .txt or .docx format and try again.",
        );
      }
    }
    ...
    const binaryCharCount = (text.match(/[\x00-\x08\x0B-\x0C\x0E-\x1F]/g) || [])
      .length;
