...
export default function EmailGenerator({ contractType, partyRole, risks }: EmailGeneratorProps) {
...
${highRisks.length > 0 ? `Priority Items Requiring Amendment:
${highRisks.map((risk, index) => `
${index + 1}. ${risk.title}
   Proposed Change: ${risk.recommendation}`).join('\n')}
` : ''}
${mediumRisks.length > 0 ? `Additional Points for Discussion:
${mediumRisks.map((risk, index) => `
${index + 1}. ${risk.title}
   Suggestion: ${risk.recommendation}`).join('\n')}
` : ''}
...
[Your Name]`;
  ...
  const getCounterparty = (contractType: string, partyRole: string) => {
    const relationships: Record<string, Record<string, string>> = {
      'Employment Agreement': { 'Employee': 'Hiring Manager', 'Employer': 'Candidate' },
      'Service Agreement': { 'Service Provider': 'Client', 'Client': 'Service Provider' },
        ...
        <div className="mt-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-500 mt-3">
              Remember to personalize this template with your specific details and availability.
            </p>
          </div>
        </div>
      )}
