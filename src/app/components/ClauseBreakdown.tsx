  ...
  const loadClauseAnalysis = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to analyze clauses: ${response.status} ${errorData.error || "Unknown error"}`,
        );
      }
      ...
      console.error(
        ...
        contractText?.substring(0, 200) || "No text",
      );
    } finally {
      setLoading(false);
    }
                  ...
                  {clause.risks.length > 0 && (
                    <span
                      className={`ml-2 inline-block w-2 h-2 rounded-full ${
                        clause.risks.some((r) => r.severity === "high")
                          ? "bg-red-500"
                          : clause.risks.some((r) => r.severity === "medium")
                            ? "bg-yellow-500"
                            : "bg-green-500"
                      }`}
                    />
                  ...
                  >
                    ...
                    {clause.risks.length > 0 && (
                      <p className="text-xs text-gray-500 mt-2">
                        {clause.risks.length} risk
                        {clause.risks.length > 1 ? "s" : ""} identified
                      </p>
                    )}
                  </div>
                ))}
                  ...
                  {clauses
                    ...
                    .map((clause) => (
                      <div key={clause.id}>
                        ...
                        {risks.filter((r) =>
                          r.clauseLocation?.includes(clause.title),
                        ).length > 0 && (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-sm text-blue-600">
                  {clauses.filter((c) => c.risks.length > 0).length} clauses
                  with risks
                </p>
                <p className="text-sm text-blue-600">
                  {clauses.reduce((sum, c) => sum + c.risks.length, 0)} total
